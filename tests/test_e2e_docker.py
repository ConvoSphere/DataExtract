...
class TestE2EExtract:
	def test_extract_text(self, http_client: httpx.Client, sample_text_file: Path) -> None:
	...
	@pytest.mark.slow
	def test_extract_pdf(self, http_client: httpx.Client, sample_pdf_file: Path) -> None:
...
@pytest.mark.e2e
class TestE2EAsync:
	def test_async_job_flow(self, http_client: httpx.Client, sample_text_file: Path) -> None:
		# Job starten
		with open(sample_text_file, "rb") as f:
